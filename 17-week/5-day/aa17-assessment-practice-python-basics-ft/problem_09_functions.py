...
class PizzaProcessor:
    ...
    def pizza_pick(self, name, crust, toppings):
        if isinstance(name, str) and isinstance(crust, str) and isinstance(toppings, int) and toppings > -1 and toppings < 11:
            cap = name.capitalize()
            lower = crust.lower()
            return f"{cap} ordered a {lower} crust pizza with {toppings} toppings!"
        else:
            return self._error
    ...
    def pizza_prepper(self, *pizzas):
        ...
        for i,p in enumerate(pizzas):
            if isinstance(p, int) and p < 11 and p > -1:
                if i % 2 == 0:
                    obj[str(i)] = { "crust": "thin", "toppings": p }
                else:
                    obj[str(i)] = { "crust": "stuffed", "toppings": p }
            else:
                return self._error
            self._pizzas_made += 1
        return obj
